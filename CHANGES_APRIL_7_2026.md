...
# 🛠️ Changes Made — April 30, 2026

---

## Overview

Fixed iOS build errors preventing the app from running on simulator/device. Resolved missing Xcode configuration files, missing framework plist, and Firebase Messaging crash on iOS.

---

## Files Changed (4 files)

### 1. `ios/Flutter/Debug.xcconfig` *(CREATED)*

**Problem:** Xcode couldn't find the base configuration file, failing the build immediately.

**Fix:** Created the file with standard Flutter iOS content:
```
...
### 2. `ios/Flutter/Release.xcconfig` *(CREATED)*

**Problem:** Same as above for Release builds.
...
### 3. `ios/Flutter/AppFrameworkInfo.plist` *(CREATED)*

**Problem:** Build script failed with `No such file or directory` when trying to package the Flutter framework.

**Fix:** Created the standard `AppFrameworkInfo.plist` with `MinimumOSVersion` set to `13.0`.

---

### 4. `lib/custom_code/actions/get_fcm_token.dart`

**Problem:** App crashed on iOS with `[firebase_messaging/apns-token-not-set] APNS token has not been set yet`.

**What changed:**
- Added `dart:io` import for `Platform` check
- Before calling `getToken()` on iOS, now checks if APNS token is available via `getAPNSToken()`
- If APNS token is null, waits 3 seconds and retries once
- Returns empty string gracefully instead of crashing
- Wrapped in try/catch for safety

---

### 5. `lib/backend/push_notifications/push_notifications_util.dart`

**Problem:** Same APNS crash but in the stream-based FCM token getter used by the auth system.

**What changed:**
- Added APNS token availability check inside the `asyncMap` callback for iOS
- Waits 3 seconds and retries if APNS token isn't immediately available
- Returns `null` (filtered out downstream) instead of crashing if APNS never arrives
...
| Action | Detail |
|--------|--------|
| `pod install` | Reinstalled all 85 CocoaPods (Pods directory was missing/incomplete) |
| `flutter clean && flutter pub get` | Full clean rebuild after fixing config files |

---

## Root Cause

The `ios/Flutter/` directory was missing essential files (`Debug.xcconfig`, `Release.xcconfig`, `AppFrameworkInfo.plist`) that are normally generated by `flutter create` but were absent — likely due to the project being exported from FlutterFlow without these standard Flutter iOS scaffolding files, or they were gitignored/deleted.
